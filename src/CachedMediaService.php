...
class CachedMediaService
{
  ...
  public function getFlySpecForSize($size,$format)
  {
    ...
    if (is_string($size))
    {
        switch ($size)
        {
          ...
          case "very_big": 
            $flySpec->setMaximumWidth(1400*$faktor);
            $flySpec->setMaximumHeight(1400);
            break;
  ...
  public function getFlySpecForVideo($quality,$format)
  {
  ...
  public function requestTranscoding($absoluteVideoPath, $entityId)
  {
    $pairs = [
      [
        'quality' => 'medium',
    ...
    {
      $values = $this->getCachedVideo($absoluteVideoPath, $entityId, $this->getFlySpecForVideo($pair['quality'],$pair['format']));
    }
